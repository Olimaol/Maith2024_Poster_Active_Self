...
fitness__array = []
for sim_id in range(params.SIMULATION_N_TOTAL):
    ### load results
    try:
...
### print best ten (max params.SIMULATION_N_TOTAL)
for i in range(min(10, len(fitness__array))):
    print(f"nr {round(fitness__array[i, 0])} with fitness {fitness__array[i, 1]}")

print(
    f"from {params.SIMULATION_SAVE_FOLDER}: nr {round(fitness__array[0, 0])} with fitness {fitness__array[0, 1]} is best"
)
